...
import {
  ...
  Film, Zap, Brain, Headphones, Bookmark, BookmarkCheck, Share2, Trash2, Cpu, Check, Clapperboard, SlidersHorizontal, Crown, UserCheck
} from "lucide-react";
        ...
        <div className="flex-1 flex gap-4 overflow-hidden min-h-0 relative">
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto space-y-4 pr-2 min-h-0 custom-scrollbar">
              ...
              {loading && (
                <div className="flex items-center gap-2 text-amber-300/80 text-xs animate-pulse p-2">
                  ...
                  <Loader2 className="w-3.5 h-3.5 animate-spin text-amber-400" /> StreamCrafters AI ამზადებს ექსკლუზიურ პასუხს...
                </div>
              )}
            </div>
            ...
            <div className="shrink-0 pt-1">
              <div className="flex items-center justify-between mb-1">
                <p className="text-[10px] text-amber-200/50 font-medium flex items-center gap-1">
                  <SlidersHorizontal className="w-3 h-3 text-amber-400" /> სწრაფი VIP იდეები:
                </p>
              </div>
            ...
            <div className="shrink-0 relative pt-1">
              {isListening && <p className="text-[10px] text-red-400 mb-1 animate-pulse text-center font-medium">🎙️ გისმენთ... ილაპარაკეთ...</p>}
              <div className="relative flex items-center">
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSend()} placeholder="ესაუბრე AI-ს, ჰკითხე რჩევა ან სთხოვე ფილმის მოძებნა..." className="w-full pl-4 pr-24 py-3 bg-black/80 border border-amber-500/30 rounded-2xl focus:outline-none focus:border-amber-400 backdrop-blur-xl text-xs md:text-sm text-amber-100 placeholder:text-amber-200/30 shadow-2xl transition-all" />
                <div className="absolute right-2 flex items-center gap-1">
                  <button onClick={handleVoiceInput} title="ხმოვანი შეყვანა" className={`p-2 rounded-xl transition-all ${isListening ? "bg-red-500 text-white animate-bounce shadow-lg" : "bg-amber-500/10 text-amber-300 hover:bg-amber-500/20 border border-amber-500/20"}`}>
